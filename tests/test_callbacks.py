...
def test_then_receives_result():
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f = pool.submit(lambda: 100).then(lambda x: x + 1)
...
def test_except_skipped_on_success():
    ...
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f = pool.submit(lambda: 42).except_(lambda e: -1)
...
def test_then_skipped_on_failure():
    ...
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        ...
        f = pool.submit(fail).then(lambda x: 999)
...
def test_finally_fires_on_success():
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f = pool.submit(lambda: 5).finally_(lambda x: x * 2)
...
def test_finally_fires_on_failure():
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        ...
        f = pool.submit(fail).finally_(lambda x: "cleaned")
...
def test_multiple_callbacks_on_same_future():
    ...
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        ...
        f1 = base.then(lambda x: x + 1)
        f2 = base.then(lambda x: x + 2)
