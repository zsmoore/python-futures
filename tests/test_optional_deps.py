...
def test_then_without_deps():
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f: Future[int] = pool.submit(lambda: 5).then(lambda x: x + 1)
...
def test_except_without_deps():
    ...
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f: Future[int] = pool.submit(boom).except_(lambda e: 99)
        assert f.result(timeout=5.0) == 99


def test_finally_without_deps():
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f: Future[int] = pool.submit(lambda: 7).finally_(lambda x: x * 2)
...
def test_explicit_empty_deps_still_works():
    with cfuture.ThreadPoolExecutor(workers=1) as pool:
        f: Future[int] = pool.submit(lambda: 3).then(lambda x: x + 1, deps=[])
