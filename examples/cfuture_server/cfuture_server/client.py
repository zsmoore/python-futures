"""
Benchmark client for the cfuture gRPC server.

Sends configurable batches of ProcessBatch RPCs and reports:
  - total wall time
  - RPC throughput (requests/sec)
  - per-batch latency (mean, p50, p95, p99)

Usage:
    python -m cfuture_server.client [--host HOST] [--port PORT]
        [--batches N] [--batch-size N] [--workers N]
"""
...
def run_benchmark(
    ...
    print(f"Benchmarking cfuture server at {address}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="cfuture gRPC benchmark client")
